...
extern int test_get_run(char *server_addr, char *client_addr_prefix, int npapp, int dims, 
	...
	size_t *elem_size, int *num_vars)
{
	...
	*dims = atoi(argv[3]);
	...
	if(argc < 3 + (*dims)*2 + 2){
		fprintf(stderr, "Wrong number of arguments!\n Usage: ./test_reader server_addr npapp dims np[0] ... np[dims-1] sp[0] ... sp[dims-1] timestep elem_size\n");
		return -1;
	}
...
int main(int argc, char **argv)
{
    ...
    // Usage: ./test_writer server_addr npapp dims np[0] ... np[dims-1] sp[0] ... sp[dims-1] timestep appid elem_size num_vars
	...
	// Run as data writer
	test_get_run(server_addr, cli_addr_prefix, npapp, dims, np,
